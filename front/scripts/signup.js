window.addEventListener('load', function () {
    ...
    const endpoint = 'http://workbench-backend:8080/users';
...
form.addEventListener('submit', function (event) {
    ...
    const role = document.getElementById('inputRole').value || null;

    if (validateInput(name, surname, email, password, shift, role)) {
        submitForm(name, surname, email, password, shift, role);
    }
});

function validateInput(name, surname, email, password, shift, role) {
...
function submitForm(name, surname, email, password, shift, role) {
    ...
    const payload = {
        ...
        password,
        shift,
        // Include role only if it's not null
        ...(role && { role })
    };
    ...
    .then(loginUserResponse => {
        ...
        sessionStorage.setItem('role', JSON.stringify(loginUserResponse.role));
