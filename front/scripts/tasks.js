...
function cancelTask(userTaskId, taskId) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed && result.value) {
            ...
            return fetch(`http://workbench-backend:8080/comments`, {
                method: 'POST',
                headers: {
        ...
        return fetch(`http://workbench-backend:8080/usertasks/${userTaskId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${sessionStorage.getItem('jwt')}`
            }
...
function initiateTask(userTaskId, taskId) {
    fetch(`http://workbench-backend:8080/usertasks`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${sessionStorage.getItem('jwt')}`
...
function completeTask(userTaskId, taskId) {
    ...
    Swal.fire({
        ...
        return fetch(`http://workbench-backend:8080/usertasks`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${sessionStorage.getItem('jwt')}`
...
function fetchTaskInfo(taskId) {
    fetch(`http://workbench-backend:8080/tasks/${taskId}`)
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
...
function fetchAndDisplayTasksCount(status, countElementId) {
    ...
    fetch(`http://workbench-backend:8080/users/${userId}`)
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
...
function fetchAndUpdateAvailableWorkCount() {
    ...
    fetch('http://workbench-backend:8080/tasks')
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
...
function submitTemplateTask(templateId) {
    fetch('../templates/tasks.json') // Replace with the actual path to your tasks.json file
        ...
        .then(templates => {
            ...
            return fetch('http://workbench-backend:8080/tasks', { // Make sure this is the correct endpoint
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
...
function updateTaskStatusToInProgress(taskId) {
    ...
    return fetch(`http://workbench-backend:8080/tasks`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
...
function fetchUsersAndCreateShiftContainers() {
    // Send a GET request to fetch the user data from 'http://workbench-backend:8080/users'
    return fetch('http://workbench-backend:8080/users')
